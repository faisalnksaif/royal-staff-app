    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <Pressable style={styles.modalBackdrop} onPress={onClose}>
        <Pressable style={[styles.modalBox, { backgroundColor: colors.background.primary }]}>
          <AppText variant="heading3" style={{ marginBottom: spacing[2] }}>Reject Testimonial</AppText>
          ...
          <TextInput
            ...
            placeholder="e.g. Too vague - please provide a specific example"
...
function PendingCard({
  ...
  return (
    <ListRow
      ...
      metaLines={[
        <AppText key="msg" variant="bodySmall" numberOfLines={3} style={{ color: colors.text.tertiary as string }}>{item.message}</AppText>,
        <AppText key="date" variant="caption" color="tertiary">{moment(item.createdAt).fromNow()}</AppText>,
      ]}
    />
  )
}
...
function ApprovedCard({ item, index }: { item: Testimonial; index?: number }) {
  const { colors } = useTheme()
  ...
  return (
    <ListRow
      ...
      metaLines={[
        <AppText key="msg" variant="bodySmall" numberOfLines={3} style={{ color: colors.text.tertiary as string }}>{item.message}</AppText>,
        <AppText key="date" variant="caption" color="tertiary">
          {item.approvedAt ? `Approved ${moment(item.approvedAt).fromNow()}` : moment(item.createdAt).fromNow()}
        </AppText>,
      ]}
    />
  )
...
// ─── TeamTestimonialsScreen ──────────────────────────────────────────────────

export default function TeamTestimonialsScreen() {
  const { colors } = useTheme()
  ...
  const { data: pendingData, isLoading: pendingLoading, refetch: refetchPending, isRefetching: refetchingPending } = useQuery({
    ...
    queryFn: () => testimonialService.getPendingTestimonials(monthParam),
  })

  const { data: approvedData, isLoading: approvedLoading, refetch: refetchApproved, isRefetching: refetchingApproved } = useQuery({
    queryKey: ["testimonials-approved", monthParam],
    queryFn: () => testimonialService.getApprovedTestimonials(monthParam),
  })
  ...
  const rejectMutation = useMutation({
    mutationFn: ({ id, reason }: { id: string; reason: string }) =>
      testimonialService.rejectTestimonial(id, reason),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["testimonials-pending", monthParam] })
  ...
  const pendingList = pendingData?.data?.testimonials ?? []
  const approvedList = approvedData?.data?.testimonials ?? []
  ...
  return (
    <View style={[styles.screen, { backgroundColor: colors.background.primary }]}>
      ...
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        {!isTablet && <BackButton />}
        <View style={{ flex: 1 }}>
          <AppText variant="heading3">Testimonials</AppText>
            ...
            pendingLoading ? (
              ...
              <View style={styles.center}>
                ...
                <AppText color="tertiary" style={{ marginTop: spacing[3] }}>
                  No pending testimonials for {month.format("MMMM YYYY")}
                </AppText>
              </View>
            )
          }
          ...
          ListEmptyComponent={
            approvedLoading ? (
              ...
              <View style={styles.center}>
                ...
                <AppText color="tertiary" style={{ marginTop: spacing[3] }}>
                  No approved testimonials for {month.format("MMMM YYYY")}
                </AppText>
              </View>
            )
          }
